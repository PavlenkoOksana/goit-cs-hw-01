...
class Parser:
    ...
    def term(self):
        """Парсер для обработки термов (множение и деление)."""
        node = self.factor()  # Используем factor как базовый терм
...
class Interpreter:
    ...
    def visit_BinOp(self, node):
        ...
        raise Exception(f'Неподдерживаемый оператор: {node.op.type}')
